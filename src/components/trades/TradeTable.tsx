...
type SortKey = 'entryDate' | 'pair' | 'side' | 'result' | 'rMultiple' | 'setup'
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-800 text-neutral-400">
              {(['entryDate', 'pair', 'side', 'setup', 'result', 'rMultiple'] as SortKey[]).map(key => (
                <th key={key} className="text-left py-3 px-3 font-medium cursor-pointer hover:text-white" onClick={() => toggleSort(key)}>
                  <div className="flex items-center gap-1">
                    {key === 'entryDate' ? 'Date' : key.charAt(0).toUpperCase() + key.slice(1)}
